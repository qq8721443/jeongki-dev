...
- 객체 지향
  `javascript function StringNumber(string){ this.string = string; } StringNumber.prototype.calculate = function () { const string = this.string; this.sum = 0; for (let i = 0; i < string.length; i++){ this.sum += parseInt(string[i]); } } const stringNumber = new StringNumber("12345"); stringNumber.calculate(); console.log(stringNumber.sum); // 15 `
